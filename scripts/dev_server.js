  ...
  function (exports_12, context_12) {
    ...
    return {
      ...
      execute: function () {
        ...
        exports_12("basename", basename = path.basename),
          exports_12("delimiter", delimiter = path.delimiter),
          exports_12("dirname", dirname = path.dirname),
          exports_12("extname", extname = path.extname),
          exports_12("format", format = path.format),
          exports_12("fromFileUrl", fromFileUrl = path.fromFileUrl),
          exports_12("isAbsolute", isAbsolute = path.isAbsolute),
          exports_12("join", join = path.join),
          exports_12("normalize", normalize = path.normalize),
          exports_12("parse", parse = path.parse),
          exports_12("relative", relative = path.relative),
          exports_12("resolve", resolve = path.resolve),
          exports_12("sep", sep = path.sep),
          exports_12(
            "toNamespacedPath",
...
System.register(
    ...
    return {
      ...
      execute: function () {
        Random = class Random {
          ...
          nextInt(min, max) {
            let rand = Number.parseFloat(
              "0." + Math.sin(this.seed++).toString().substr(6),
            );
            min = Math.ceil(min);
            max = Math.floor(max);
        ...
        HEADER_CONTENT_TYPE_PLAIN = { "Content-type": "text/plain" };
        HEADER_CONTENT_TYPE_SVG = { "Content-type": "image/svg+xml" };
        RSP_200 = () => {
          return {
            status: 200,
            headers: new Headers(HEADER_CONTENT_TYPE_PLAIN),
            body: "200 OK",
          };
        };
        RSP_404 = () => {
        ...
        generateRandomAppList = (seed = Math.PI * Math.PI, size = 50) => {
          let apps = [];
          let colors = {};
          let r = new Random(seed);
          for (let i = 0; i < size; i++) {
            let length = r.nextInt(1, 4); // name word count [1, 4)
            let app = { name: "", activity: "", package: "" };
        ...
        // server service
        _a = generateRandomAppList(), APP_LIST = _a[0], APP_THEME = _a[1];
        addCORSHeader = (requestHeaders, responseHeaders) => {
          ...
          if (requestHeaders.has("Access-Control-Request-Methods")) {
            let accessMethod = requestHeaders.get(
          ...
          if (requestHeaders.has("Access-Control-Request-Headers")) {
            let accessHeader = requestHeaders.get(
        ...
        doGetApplicationList = async (req) => {
          let rsp = { status: 200 };
          rsp.headers = new Headers(HEADER_CONTENT_TYPE_JSON);
          rsp.body = JSON.stringify(APP_LIST);
          return rsp;
        };
        doGetApplicationIcon = async (req) => {
          let activityName = getParameterByName(req.url, "activity");
  ...
  <g>
   <rect x="-1" y="-1" width="${size + 2}" height="${size +
              2}" id="canvas_background" fill="none"/>
  </g>
  <g>
   <ellipse ry="${size / 2.2}" rx="${size / 2.2}" id="svg_3" cy="${size /
              2}" cx="${size / 2}" stroke-width="0" stroke="#000" fill="${
              theme[0]
            }"/>
   <ellipse ry="${size / 3}" rx="${size / 3}" id="svg_3" cy="${size /
              2}" cx="${size / 2}" stroke-width="0.5" stroke="#000" fill="${
              theme[1]
            }"/>
   <ellipse ry="${size / 5}" rx="${size / 5}" id="svg_3" cy="${size /
              2}" cx="${size / 2}" stroke-width="0" stroke="#000" fill="${
              theme[2]
            }"/>
  </g>
        ...
        doRequestPermission = async (req) => {
          let name = getParameterByName(req.url, "name");
        ...
        // start dev server
        __serverArgs = mod_ts_5.parse(Deno.args);
        __port = __serverArgs.port ?? __serverArgs.p ?? 10801;
        console.log(`Launcher server listening on http://0.0.0.0:${__port}/`);
        server_ts_2.listenAndServe(
          { hostname: "0.0.0.0", port: __port },
          handler,
        );
      },
    };
